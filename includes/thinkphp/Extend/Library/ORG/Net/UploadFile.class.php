...
{ //类定义开始
	 ...
	 * @throws ThinkExecption
	 +----------------------------------------------------------
	   @internal
	     $fileType 20131115修改 当值为image时(即上传文件明确了是图片)，获取大小
	 */
	public function upload($savePath = '', $fileType = '')
	{
		//如果不指定保存文件名，则由系统默认
		if (empty ( $savePath ))
		{
		...
		foreach ( $files as $key => $file )
		{
			...
			if (! empty ( $file ['name'] ))
			{
				...
				if($fileType=='image')
				{
					$imgF = getimagesize ( $file ['tmp_name'] );
					$file['width'] = $imgF[0];
					$file['height'] = $imgF[1];
					$imgF = NULL;					
				}
	 ...
	 * @throws ThinkExecption
	 +----------------------------------------------------------
	   $fileType 20131115修改 当值为image时(即上传文件明确了是图片)，获取大小
	 */
	public function uploadOne($file, $savePath = '',$fileType = '')
	{
		//如果不指定保存文件名，则由系统默认
		if (empty ( $savePath ))
			$savePath = $this->savePath;
